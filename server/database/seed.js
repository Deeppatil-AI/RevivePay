import { db, sqlite } from './store.js';
import { generateFullBatch } from '../../src/data/syntheticBatch.js';
import { fileURLToPath } from 'url';
import path from 'path';

export function seedDatabase(force = false) {
  const subCount = sqlite.prepare('SELECT count(*) as count FROM subscriptions').get().count;
  
  if (subCount > 0 && !force) {
    console.log(`ℹ️  SQLite database already contains ${subCount} subscriptions. Skipping seed.`);
    return;
  }

  console.log('🌱 Seeding SQLite database with real Indian enterprise datasets...');

  // 1. Seed Subscriptions Cohort
...
if (path.resolve(currentFile) === executedFile || process.argv.includes('--run')) {
  seedDatabase(true);
}
