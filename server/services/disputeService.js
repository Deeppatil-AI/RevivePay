...
export function compileDisputeDossier(disputeId) {
  ...
  dispute.status = "SUBMITTED_TO_NETWORK";
  dispute.dossierCompiled = true;
  db.updateDispute(dispute);

  const winRate = dispute.winProbability ? Math.round(dispute.winProbability <= 1 ? dispute.winProbability * 100 : dispute.winProbability) : 92;
  ...
  const auditEntry = {
    ...
    diagnosis: {
      ...
      detailedRationale: `Customer raised chargeback (${dispute.reasonCode}), but OTP 3DS authentication & delivery proof confirm genuine receipt.`
    },
    policy: {
      status: "APPROVED",
    ...
    actionDetail: `Auto-submitted 4-point evidentiary packet (${packetId}) via Razorpay Dispute API. Predicted Win Rate: ${winRate}%.`,
